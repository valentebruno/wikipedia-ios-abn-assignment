...
@interface WMFPlacesDeepLinkCoordinator : NSObject
- (void)navigateUsingActivity:(NSUserActivity *)activity placesViewController:(WMFPlacesViewController *)placesViewController;
@end

@implementation WMFPlacesDeepLinkCoordinator

- (void)navigateUsingActivity:(NSUserActivity *)activity placesViewController:(WMFPlacesViewController *)placesViewController {
    [placesViewController updateViewModeToMap];

    NSNumber *latitudeNumber = activity.wmf_placesLatitude;
    NSNumber *longitudeNumber = activity.wmf_placesLongitude;
    if (latitudeNumber != nil && longitudeNumber != nil) {
        ...
        if ([self isValidLatitude:latitude longitude:longitude]) {
            [placesViewController showCoordinateWithLatitude:latitude longitude:longitude];
            return;
        }
    }
...
- (BOOL)isValidLatitude:(double)latitude longitude:(double)longitude {
...
@property (nonatomic, strong) WMFPlacesDeepLinkCoordinator *placesDeepLinkCoordinator;
...
- (instancetype)init {
    ...
    if (self) {
        ...
        _placesDeepLinkCoordinator = [[WMFPlacesDeepLinkCoordinator alloc] init];
    }
    return self;
}
...
- (BOOL)processUserActivity:(NSUserActivity *)activity animated:(BOOL)animated completion:(dispatch_block_t)done {
    ...
    switch (type) {
        ...
        case WMFUserActivityTypePlaces: {
            ...
            [self.currentTabNavigationController popToRootViewControllerAnimated:animated];
            WMFPlacesViewController *placesViewController = [self placesViewController];
            [self.placesDeepLinkCoordinator navigateUsingActivity:activity placesViewController:placesViewController];
        } break;
        case WMFUserActivityTypeContent: {
            [self dismissPresentedViewControllers];
            [self setSelectedIndex:WMFAppTabTypeMain];
