...
static NSString *const WMFPlacesLatitudeUserInfoKey = @"WMFPlacesLatitude";
static NSString *const WMFPlacesLongitudeUserInfoKey = @"WMFPlacesLongitude";
...
static NSNumber *_Nullable wmf_numberFromCoordinateString(NSString *_Nullable value) {
    if (value == nil) {
        return nil;
    }
...
static BOOL wmf_isValidLatitude(double lat) {
    return lat >= -90.0 && lat <= 90.0;
}

static BOOL wmf_isValidLongitude(double lon) {
...
+ (instancetype)wmf_placesActivityWithURL:(NSURL *)activityURL {
    ...
    for (NSURLQueryItem *item in components.queryItems) {
        ...
        if ([itemName isEqualToString:@"wmfarticleurl"]) {
            NSString *articleURLString = item.value;
            articleURL = [NSURL URLWithString:articleURLString];
            continue;
        }
        if ([itemName isEqualToString:@"lat"] || [itemName isEqualToString:@"latitude"]) {
            NSNumber *parsed = wmf_numberFromCoordinateString(item.value);
            if (parsed != nil && wmf_isValidLatitude(parsed.doubleValue)) {
                latitude = parsed;
            }
            continue;
        }
        if ([itemName isEqualToString:@"lon"] || [itemName isEqualToString:@"long"] || [itemName isEqualToString:@"lng"] || [itemName isEqualToString:@"longitude"]) {
            NSNumber *parsed = wmf_numberFromCoordinateString(item.value);
            if (parsed != nil && wmf_isValidLongitude(parsed.doubleValue)) {
                longitude = parsed;
            }
        }
    }
    ...
    if (latitude != nil && longitude != nil) {
        ...
        userInfo[WMFPlacesLatitudeUserInfoKey] = latitude;
        userInfo[WMFPlacesLongitudeUserInfoKey] = longitude;
...
- (nullable NSNumber *)wmf_placesLatitude {
    NSNumber *latitude = self.userInfo[WMFPlacesLatitudeUserInfoKey];
...
- (nullable NSNumber *)wmf_placesLongitude {
    NSNumber *longitude = self.userInfo[WMFPlacesLongitudeUserInfoKey];
